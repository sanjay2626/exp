...
for($i=0;$i<$n9;$i++){
 $steminfra9=$row9[$i];
  $stem_models_data123 = "select DISTINCT school.id, school.city, school.name, stemlabinfrastc.EWork_progress, stemlabinfrastc.modelDesks_progress, stemlabinfrastc.cupboard_progress, stemlabinfrastc.Solar_progress, stem_models_data.science_progress, stem_models_data.math_progress, stem_models_data.robotics_progress, stem_models_data.computer_progress, stempostersdata.bWall_progress, stempostersdata.concepts_progress, stempostersdata.sSystem_progress, stempostersdata.inCorner_progress, stempostersdata.cutouts_progress, stem_models_data.user_id, stemlabinfrastc.user_id, stempostersdata.user_id, innaugration.InnaugrationDate , stemimpactassessment.StemImpactDate, teachertraining.teacherTrainingDate, teachertraining.teacherTraining_eDate  from school 
  left join stempostersdata on school.id=stempostersdata.schoolid
  left join stemlabinfrastc on school.id=stemlabinfrastc.schoolid
  ...
  where stempostersdata.schoolid = '".$steminfra9['school_id']."' OR stemlabinfrastc.schoolid = '".$steminfra9['school_id']."' OR stem_models_data.schoolid = '".$steminfra9['school_id']."' OR innaugration.schoolid = '".$steminfra9['school_id']."' order by school.city";
...
if($row_stem_models_data123['EWork_progress']=="NA"){$e1='0';}else{$e1='1';}
if($row_stem_models_data123['modelDesks_progress']=="NA"){$e2='0';}else{$e2='1';}
if($row_stem_models_data123['cupboard_progress']=="NA"){$e3='0';}else{$e3='1';}
if($row_stem_models_data123['Solar_progress']=="NA"){$e4='0';}else{$e4='1';}
$e1total = $e1+$e2+$e3+$e4;
$infra = $row_stem_models_data123['EWork_progress']+$row_stem_models_data123['modelDesks_progress']+$row_stem_models_data123['cupboard_progress']+$row_stem_models_data123['Solar_progress'];
...
if($cekinstansi9!=$steminfra9['city']){
   ...
   ?>
    ...
    <!-- <?php  if(!empty($row_stem_models_data123['StemImpactDate'])){ $date = date('d/m/Y',strtotime($row_stem_models_data123['StemImpactDate']));  if($date<=date('d/m/Y')){ echo "background-color: #70AD47;";  }else{ echo "background-color: #1DA29F;";   } }else{ echo "background-color: #F83E6A;"; } ?>"><?php if(!empty($row_stem_models_data123['StemImpactDate'])){echo date('d/m/Y',strtotime($row_stem_models_data123['StemImpactDate']));}else{ echo "NA"; } ?></th>-->
     
    <th class="fontcolor" style="text-align: center;"><a target="_blank" href="stemgallery2.php?schoolid=<?php echo $steminfra9['school_id']; ?>&user=<?php echo $row_stem_models_data123['user_id']; ?>&sno=<?php echo $steminfra9['sno']; ?>"><img src="https://i.postimg.cc/zvhWkmCP/imgicon.png" class="wpimg"></th>
  </tr>
<?php  }  ?>
